classdef FactorizedMatrix
    ...
    methods
        ...
        function this=FactorizedMatrix(original,dim1vectors,dim2vectors,factMethod,name) %Last two are optional
            ...
            if ~isempty(original) && any(auxSize~=size(original))
        ...
        function [figHandle,plotHandles1,plotHandles2]=plot(this,plotHandles1,plotHandles2,colors)
            ...
            if nargin<3 || isempty(plotHandles1) || isempty(plotHandles2) %No handles
                ...
                if (length(plotHandles1)==this.reducedDimension) && (length(plotHandles1)==this.reducedDimension)
    ...
    methods(Static)
        ...
        function newObj=factorize(matrix,dimInd,method,newDim,name) 
            ...
            switch method
                ...
                case 'pca' %No mean substraction, factorization
                    [tensor1, tensor2, LATENT, TSQUARED, EXPLAINED, MU]=pca(aux,'Centered','false');
                    %To Do
                    tensor1=reshape(tensor1',[newDim matSize(1:dimInd) ]);
                    tensor2=reshape(tensor2,[newDim matSize(dimInd+1:end)]);
                case 'ica'
                ...
                case 'pica'
                    ...
                    tensor2=[];
                case 'ppca' 
                    %Invented method: returns a strictly positive base 
                    %for the subspace returned by pca. 
                    %Projection is then done with a positivity constraint on coefficients.
                    %To Do
                    tensor1=[];
                    tensor2=[];
                otherwise
                    tensor1=[];
                    tensor2=[];
                    error('FactorizedMatrix:Factorize','Unknown method.')
            end
