function [J,Xh,V,K,r2] = estimateDynv3b(X, realPolesOnly, nullK, J0)
...
if realPolesOnly % Optimize to find best decaying exponential fits:
    %Bounds & options:
    lb=[zeros(size(t0))];
    ub=[(5*NN*ones(size(t0)))]; %This limits time-constants so that the longest does not converge to a line.
...
r2=bestRes; %Should be equivalent to 1-norm(E,'fro')^2/norm(X,'fro')^2, E= (I-P'*P)*X
end
